  ...
  class TagInfo
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DevStar::TagInfo` initialize method"
      end
    ...
    def list_invalid_properties
      ...
      if !@name.empty? && @name.to_s.length > 200
    ...
    def valid?
      ...
      return false if !@id.empty? && @id.to_s.length < 32
      return false if !@name.empty? && @name.to_s.length > 200
