  ...
  class JobResp
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DevStar::JobResp` initialize method"
      end
    ...
    def list_invalid_properties
      ...
      if !@jobId.empty? && @jobId.to_s.length > 8
    ...
    def valid?
      return false if !@jobId.empty? && @jobId.to_s.length > 8
      return false if !@jobId.empty? && @jobId.to_s.length < 8
      true
    end
