  ...
  class Reference
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DevStar::Reference` initialize method"
      end
    ...
    def list_invalid_properties
      ...
      if !@title.empty? && @title.to_s.length > 2048
        invalid_properties.push('invalid value for "title", the character length must be smaller than or equal to 2048.')
      end

      if !@title.empty? && @title.to_s.length < 0
        invalid_properties.push('invalid value for "title", the character length must be great than or equal to 0.')
      end

      if !@url.empty? && @url.to_s.length > 2048
        invalid_properties.push('invalid value for "url", the character length must be smaller than or equal to 2048.')
      end

      if !@url.empty? && @url.to_s.length < 0
    ...
    def valid?
      return false if !@title.empty? && @title.to_s.length > 2048
      return false if !@title.empty? && @title.to_s.length < 0
      return false if !@url.empty? && @url.to_s.length > 2048
      return false if !@url.empty? && @url.to_s.length < 0
    ...
    def title=(title)
      if !title.empty? && title.to_s.length > 2048
        fail ArgumentError, 'invalid value for "title", the character length must be smaller than or equal to 2048.'
      end
    ...
    def hash
      [title, url, type, productshort].hash
    end
