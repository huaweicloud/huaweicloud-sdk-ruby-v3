  ...
  class PipelineTemplateInfo
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DevStar::PipelineTemplateInfo` initialize method"
      end
    ...
    def list_invalid_properties
      ...
      if !@name.empty? && @name.to_s.length > 64
      ...
      if !@detail.empty? && @detail.to_s.length > 128
        invalid_properties.push('invalid value for "detail", the character length must be smaller than or equal to 128.')
      end

      if !@detail.empty? && @detail.to_s.length < 0
        invalid_properties.push('invalid value for "detail", the character length must be great than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if !@id.empty? && @id.to_s.length < 32
      return false if !@name.empty? && @name.to_s.length > 64
      return false if !@name.empty? && @name.to_s.length < 1
      return false if !@detail.empty? && @detail.to_s.length > 128
      return false if !@detail.empty? && @detail.to_s.length < 0
      true
    end
    ...
    def detail=(detail)
      if !detail.empty? && detail.to_s.length > 128
        fail ArgumentError, 'invalid value for "detail", the character length must be smaller than or equal to 128.'
      end

      if !detail.empty? && detail.to_s.length < 0
        fail ArgumentError, 'invalid value for "detail", the character length must be great than or equal to 0.'
      end
